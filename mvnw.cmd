...
@REM set %HOME% to equivalent of C:\Users\TELUKUNTLA
...
@REM Try to auto-detect Java
where java >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    @REM Try to find JDK installation (prefer jdk over jre)
    ...
    @REM Fallback: find from java.exe path
    for /f "delims=" %%i in ('where java') do set JAVA_EXE=%%i
    for %%i in ("%JAVA_EXE%") do (
        set JAVA_DIR=%%~dpi
        if "%JAVA_DIR:~-9%"=="\javapath\" (
            @REM It's a symlink, try to find actual JDK
